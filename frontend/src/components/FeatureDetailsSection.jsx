...
function FeatureRow({ item, isOpen, onToggle }) {
  return (
    <div className="border-b border-gray-300 py-3.5">
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between text-left gap-4"
      >
        <span className="text-lg font-medium text-[#3A3A3A]">{item.title}</span>
        <span className="text-2xl leading-none text-[#4B5375]">
          {isOpen ? "−" : "+"}
        </span>
      </button>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <Motion.p
            ...
            className="overflow-hidden pt-2.5 text-sm leading-relaxed text-gray-600 pr-8"
          >
            {item.description}
          </Motion.p>
        )}
    ...
    <section className="max-w-7xl mx-auto px-6 pt-16 pb-10">
      <div className="grid md:grid-cols-2 gap-12 items-center">
        ...
        <Motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-3xl md:text-4xl font-bold leading-tight text-black mb-4">
            Plan Your Perfect Room Layout
          </h2>

          <p className="text-gray-700 text-base md:text-lg leading-relaxed max-w-[560px] mb-6">
          ...
          <button
            ...
            className="mt-6 inline-flex items-center gap-2 rounded-md bg-[#1F5A2E] px-6 py-2.5 text-sm font-medium text-white transition hover:bg-[#174724]"
          >
